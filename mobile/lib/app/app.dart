...
class _TodoAppState extends State<TodoApp> {
  ...
  @override void initState() { super.initState(); _taskStore=TaskStore(); LocalNotificationService.onNotificationTap=_openTask; _loadFuture=_taskStore.load(); }
  void _openTask(String id) { final ctx=_navigatorKey.currentContext; if(ctx==null)return; for(final task in _taskStore.tasks){if(task.id==id){Navigator.of(ctx).push(MaterialPageRoute(builder:(_)=>TaskDetailPage(store:_taskStore,task:task)));break;}} }
  @override void dispose(){LocalNotificationService.onNotificationTap=null;_taskStore.dispose();super.dispose();}
  @override Widget build(BuildContext context)=>MaterialApp(navigatorKey:_navigatorKey,title:'Todo',debugShowCheckedModeBanner:false,theme:AppTheme.light,darkTheme:AppTheme.dark,themeMode:ThemeMode.system,themeAnimationDuration:const Duration(milliseconds:350),themeAnimationCurve:Curves.easeOutCubic,home:FutureBuilder<void>(future:_loadFuture,builder:(context,snapshot){if(snapshot.connectionState!=ConnectionState.done)return const _Loading();if(snapshot.hasError)return _Error(onRetry:()=>setState(()=>_loadFuture=_taskStore.load()));return HomePage(store:_taskStore);}));
}
