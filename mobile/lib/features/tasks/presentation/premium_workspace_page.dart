...
class _PremiumWorkspacePageState extends State<PremiumWorkspacePage> {
  ...
  void _add() {
    showModalBottomSheet<void>(
      ...
      showDragHandle: true,
      builder: (_) => _QuickAdd(store: widget.store),
    );
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not add the task.')));
      }
    }
  }
                ...
                child: OutlinedButton.icon(
                  onPressed: _busy
                      ...
                      : () {
                          ...
                          Navigator.push(context, MaterialPageRoute(builder: (_) => AddTaskPage(store: widget.store)));
                        },
        ...
        slivers: [
          ...
          if (overdue > 0)
            SliverPadding(
              ...
              sliver: SliverToBoxAdapter(child: Text('$overdue overdue ${overdue == 1 ? 'task' : 'tasks'}', style: TextStyle(color: Theme.of(context).colorScheme.error, fontWeight: FontWeight.w800))),
            ),
          SliverPadding(
            padding: const EdgeInsets.fromLTRB(22, 24, 22, 10),
            sliver: SliverToBoxAdapter(
              child: Row(children: [Expanded(child: Text('Today’s plan', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900))), Text('${today.length}', style: Theme.of(context).textTheme.bodySmall)]),
            ),
          ),
          if (today.isEmpty)
            SliverPadding(padding: const EdgeInsets.fromLTRB(22, 5, 22, 110), sliver: SliverToBoxAdapter(child: _Empty(onAdd: onAdd)))
          else
            SliverPadding(
              padding: const EdgeInsets.fromLTRB(22, 0, 22, 110),
              sliver: SliverList.separated(itemCount: today.length, itemBuilder: (_, i) => _TaskTile(task: today[i], store: store), separatorBuilder: (_, __) => const SizedBox(height: 9)),
            ),
        ],
      ),
    );
...
class _Progress extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          SizedBox(width: 76, height: 76, child: Stack(alignment: Alignment.center, children: [CircularProgressIndicator(value: progress, strokeWidth: 6, backgroundColor: scheme.onPrimary.withValues(alpha: .18), color: scheme.onPrimary), Text('${(progress * 100).round()}%', style: TextStyle(color: scheme.onPrimary, fontWeight: FontWeight.w900))])),
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(21), border: Border.all(color: scheme.outlineVariant.withValues(alpha: .45))),
          child: Row(
            children: [
              GestureDetector(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 5),
                    if (task.dueAt != null) Text(TimeOfDay.fromDateTime(task.dueAt!).format(context), style: TextStyle(fontSize: 11.5, color: accent, fontWeight: FontWeight.w700)),
                  ],
                ),
              ),
