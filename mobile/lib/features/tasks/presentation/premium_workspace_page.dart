...
import 'home_page.dart' show FocusView, SettingsView;
...
class _Today extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final overdue = active.where((task) => task.dueAt != null && task.dueAt!.isBefore(now)).toList()..sort(_sortDue);
    ...
    return SafeArea(
      child: CustomScrollView(slivers: [
        SliverPadding(
          ...
          sliver: SliverToBoxAdapter(child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(_dateLabel(now), style: TextStyle(color: scheme.onSurfaceVariant, fontSize: 13, fontWeight: FontWeight.w700)),
              const SizedBox(height: 5),
              Text('Today', style: Theme.of(context).textTheme.displaySmall?.copyWith(fontWeight: FontWeight.w900, letterSpacing: -1.4)),
            ])),
            IconButton(onPressed: onSearch, style: IconButton.styleFrom(backgroundColor: scheme.surfaceContainerHighest), icon: const Icon(Icons.search_rounded)),
          ])),
        ),
        ...
        SliverPadding(padding: const EdgeInsets.fromLTRB(22, 25, 22, 10), sliver: SliverToBoxAdapter(child: Row(children: [
        ...
        if (overdue.isNotEmpty) SliverPadding(padding: const EdgeInsets.fromLTRB(22, 0, 22, 10), sliver: SliverToBoxAdapter(child: _Overdue(count: overdue.length))),
        if (today.isEmpty)
          SliverPadding(padding: const EdgeInsets.fromLTRB(22, 4, 22, 110), sliver: SliverToBoxAdapter(child: _EmptyToday(onAdd: onAdd)))
        else
          SliverPadding(padding: const EdgeInsets.fromLTRB(22, 0, 22, 110), sliver: SliverList.separated(
  ...
  static String _dateLabel(DateTime date) => '${const ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][date.weekday - 1]}, ${date.day} ${const ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'][date.month - 1]}';
}
    ...
    return Container(
      ...
      child: Row(children: [
        SizedBox(width: 78, height: 78, child: Stack(alignment: Alignment.center, children: [
          ...
          Text('${(progress * 100).round()}%', style: TextStyle(color: scheme.onPrimary, fontSize: 17, fontWeight: FontWeight.w900)),
        ])),
        ...
        child: Container(
          ...
          child: Row(children: [
            GestureDetector(
              ...
              child: AnimatedContainer(
                duration: const Duration(milliseconds: 180), width: 27, height: 27,
...
class _Overdue extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 11), decoration: BoxDecoration(color: scheme.errorContainer.withValues(alpha: .65), borderRadius: BorderRadius.circular(16)), child: Row(children: [Icon(Icons.priority_high_rounded, size: 18, color: scheme.error), const SizedBox(width: 9), Text('$count overdue ${count == 1 ? 'task' : 'tasks'}', style: TextStyle(fontWeight: FontWeight.w800, color: scheme.onErrorContainer)), const Spacer(), Icon(Icons.arrow_forward_rounded, size: 17, color: scheme.error)]));
  }
}
...
class _EmptyToday extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return Container(padding: const EdgeInsets.all(26), decoration: BoxDecoration(color: scheme.surfaceContainerHighest.withValues(alpha: .55), borderRadius: BorderRadius.circular(24)), child: Column(children: [
      ...
      const SizedBox(height: 14), Text('Nothing on your schedule', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900)), const SizedBox(height: 5),
      const Text('Your day is clear. Add something you want to accomplish.', textAlign: TextAlign.center), const SizedBox(height: 16),
