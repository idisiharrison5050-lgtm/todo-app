      ...
      builder: (context, _) {
        ...
        final children = <Widget>[
          Card(child: Padding(padding: const EdgeInsets.all(20), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Checkbox(value: task.isCompleted, onChanged: (_) => widget.store.toggleCompleted(task.id)),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (task.isFavorite) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Icon(Icons.star_rounded, size: 17, color: scheme.primary), const SizedBox(width: 5), Text('Favorite', style: TextStyle(color: scheme.primary, fontWeight: FontWeight.w700))]))
            ])),
          ]))),
          ...
          if (task.tags.isNotEmpty) ...[const SizedBox(height: 12), _InfoCard(icon: Icons.tag, title: 'Tags', child: Wrap(spacing: 6, runSpacing: 6, children: task.tags.map<Widget>((tag) => Chip(label: Text(tag))).toList()))],
          ...
          const SizedBox(height: 20),
          _SectionCard(title: 'Activity', icon: Icons.history_rounded, trailing: Text('${task.history.length}', style: const TextStyle(fontWeight: FontWeight.w800)), child: task.history.isEmpty ? const Text('No activity recorded yet.') : Column(children: task.history.reversed.take(12).map<Widget>((entry) => _HistoryRow(entry: entry)).toList())),
        ...
        return Scaffold(
          ...
          body: ListView(padding: const EdgeInsets.fromLTRB(20, 12, 20, 40), children: children),
        );
      },
    );
  }
...
class _SubtaskRow extends StatelessWidget { const _SubtaskRow({required this.taskId, required this.item, required this.store}); final String taskId; final TaskSubtask item; final TaskStore store; @override Widget build(BuildContext context) => ListTile(contentPadding: EdgeInsets.zero, leading: Checkbox(value: item.isCompleted, onChanged: (value) => store.updateSubtask(taskId, item.id, isCompleted: value ?? false)), title: Text(item.title, style: TextStyle(decoration: item.isCompleted ? TextDecoration.lineThrough : null, fontWeight: FontWeight.w600)), trailing: IconButton(tooltip: 'Delete checklist item', icon: const Icon(Icons.close_rounded), onPressed: () => store.deleteSubtask(taskId, item.id))); }
class _HistoryRow extends StatelessWidget { const _HistoryRow({required this.entry}); final TaskHistoryEntry entry; @override Widget build(BuildContext context) => Padding(padding: const EdgeInsets.only(bottom: 12), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.circle, size: 8), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(entry.action, style: const TextStyle(fontWeight: FontWeight.w700)), if (entry.detail.isNotEmpty) Text(entry.detail), Text(_format(entry.timestamp), style: Theme.of(context).textTheme.bodySmall)]))])); String _format(DateTime value) => '${value.day}/${value.month}/${value.year} • ${value.hour.toString().padLeft(2, '0')}:${value.minute.toString().padLeft(2, '0')}'; }
class _SectionCard extends StatelessWidget { const _SectionCard({required this.title, required this.icon, required this.child, this.trailing}); final String title; final IconData icon; final Widget child; final Widget? trailing; @override Widget build(BuildContext context) { final header = <Widget>[Icon(icon), const SizedBox(width: 10), Expanded(child: Text(title, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)))]; if (trailing != null) header.add(trailing!); return Card(elevation: 0, child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: header), const SizedBox(height: 14), child]))); } }
