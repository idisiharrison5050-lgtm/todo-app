...
class TaskDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final current = store.tasks.where((value) => value.id == task.id).firstOrNull ?? task;
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.of(context).push(
              MaterialPageRoute(builder: (_) => AddTaskPage(store: store, task: current)),
            ),
          ),
          IconButton(
            tooltip: 'Delete task',
      ...
      body: ListView(
        ...
        children: [
              ...
              child: Row(
                ...
                children: [
                  Checkbox(
                    ...
                    onChanged: (_) => store.toggleCompleted(current.id),
                  ),
          ...
          _InfoCard(
            ...
            title: 'Schedule',
            child: Text(
              current.dueAt == null ? 'No due date' : _formatDateTime(context, current.dueAt!),
          ...
          _InfoCard(
            ...
            title: 'Reminder',
            child: Text(
              current.reminderType == TaskReminderType.none
                  ? 'No reminder'
                  : current.reminderType == TaskReminderType.once
                      ? 'Remind once'
                      : 'Every ${_formatDuration(current.reminderInterval)}',
              style: Theme.of(context).textTheme.bodyLarge,
            ),
          ),
          const SizedBox(height: 12),
          _InfoCard(
            ...
            title: 'Priority',
            child: Text(
              _priorityLabel(current.priority),
              style: Theme.of(context).textTheme.bodyLarge?.copyWith(
                    fontWeight: FontWeight.w700,
                    color: current.priority == TaskPriority.high ? scheme.error : null,
                  ),
            ),
          ),
          const SizedBox(height: 28),
          FilledButton.icon(
            onPressed: () => store.toggleCompleted(current.id),
            icon: Icon(current.isCompleted ? Icons.undo : Icons.check),
            label: Text(current.isCompleted ? 'Mark active' : 'Mark completed'),
          ),
        ],
      ),
    );
