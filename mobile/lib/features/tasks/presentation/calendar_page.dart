...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Widget build(BuildContext context) {
    ...
    for (final d in ['M', 'T', 'W', 'T', 'F', 'S', 'S']) {
      weekdays.add(Expanded(child: Center(child: Text(d, style: theme.textTheme.labelMedium?.copyWith(fontWeight: FontWeight.w800)))));
    }
    return Scaffold(
      appBar: AppBar(title: const Text('Calendar')),
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 36), children: [
        Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
          Row(children: [IconButton(onPressed: () => setState(() => _month = DateTime(_month.year, _month.month - 1)), icon: const Icon(Icons.chevron_left_rounded)), Expanded(child: Text(monthName, textAlign: TextAlign.center, style: theme.textTheme.titleLarge)), IconButton(onPressed: () => setState(() => _month = DateTime(_month.year, _month.month + 1)), icon: const Icon(Icons.chevron_right_rounded))]),
          const SizedBox(height: 8), Row(children: weekdays), const SizedBox(height: 8),
          GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: cells.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 7, mainAxisExtent: 58), itemBuilder: (_, i) {
            ...
            final selected = _sameDay(date, _selected); final today = _sameDay(date, DateTime.now());
