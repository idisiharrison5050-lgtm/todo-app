...
class _PremiumSettingsPageState extends State<PremiumSettingsPage> {
        ...
        children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        user?.name.isNotEmpty == true ? 'Welcome, ${user!.name}' : 'Your workspace',
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: TextStyle(color: scheme.onPrimary, fontSize: 18, fontWeight: FontWeight.w900),
                      ),
          ...
          _Section(
            ...
            children: [
              _SettingTile(
                ...
                title: 'Appearance',
                subtitle: _themeLabel(scope.themeMode),
                onTap: () => _showAppearance(context, scope),
              ),
              _SettingTile(
                icon: Icons.notifications_none_rounded,
                title: 'Notifications',
                subtitle: 'Request permission and send a test alert',
                onTap: () => _showNotifications(context, scope.notifications),
              ),
              _SettingTile(
                icon: Icons.sync_rounded,
                title: 'Sync & offline',
                subtitle: 'Refresh from your local workspace and cloud',
                onTap: () => _syncNow(context, scope.store),
              ),
              _SettingTile(
                icon: Icons.cleaning_services_outlined,
                title: 'Clear completed',
                subtitle: 'Remove completed tasks from this workspace',
                onTap: () => _clearCompleted(context, scope.store),
              ),
            ],
          ),
          ...
          _Section(
            ...
            children: [
              _SettingTile(
                ...
                title: 'Privacy',
                subtitle: 'Your task data is isolated to your account',
                onTap: () => _showPrivacy(context),
              ),
              _SettingTile(
                icon: Icons.person_outline_rounded,
                title: 'Account',
                subtitle: user?.email.isNotEmpty == true ? user!.email : 'Signed-in account',
                onTap: () => _showAccount(context, scope.authStore),
              ),
            ],
          ),
          const SizedBox(height: 18),
          Card(
            child: ListTile(
              ...
              onTap: _busy ? null : () => _confirmLogout(context),
            ),
          ),
          const SizedBox(height: 18),
          Center(
            child: Text(
              'Todo • Built for getting things done',
        ...
        child: Column(
          ...
          children: [
            const ListTile(
              ...
              title: Text('Notifications', style: TextStyle(fontWeight: FontWeight.w900)),
              subtitle: Text('Use this screen to verify Todo can alert this device.'),
            ),
            ListTile(
              leading: const Icon(Icons.verified_outlined),
              title: const Text('Request notification permission'),
              subtitle: const Text('Ask Android or iOS to allow Todo alerts'),
              onTap: () => Navigator.pop(sheetContext, 'permission'),
            ),
            ListTile(
              leading: const Icon(Icons.notification_add_outlined),
              title: const Text('Send test notification'),
              subtitle: const Text('Schedules a test alert about 5 seconds from now'),
  ...
  Future<void> _clearCompleted(BuildContext context, TaskStore store) async {
    ...
    if (completed == 0) {
      _showMessage(context, 'There are no completed tasks to clear.');
      return;
    }
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text('Remove $completed completed task${completed == 1 ? '' : 's'} from this workspace?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Cancel')),
          FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Clear completed')),
        ],
      ),
    );
    if (confirmed == true) {
      await store.clearCompleted();
      if (context.mounted) _showMessage(context, 'Completed tasks cleared.');
    }
  }
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(user?.name.isNotEmpty == true ? user!.name : 'Signed-in user', style: const TextStyle(fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
            Text(user?.email.isNotEmpty == true ? user!.email : 'Account email unavailable'),
            const SizedBox(height: 14),
            const Text('Your tasks are stored against your authenticated account and are protected by the server-side ownership checks.'),
          ],
        ),
  ...
  Future<void> _showPrivacy(BuildContext context) async {
    await showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text('Todo keeps task data scoped to the signed-in account. Local offline data is kept on the device so you can continue working without a connection. Notification actions only carry the task identifier needed to open or snooze the task.'),
        actions: [TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Done'))],
      ),
    );
  }
...
class _ThemeOption extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListTile(
        ...
        title: Text(label),
        trailing: mode == selected ? const Icon(Icons.check_rounded) : null,
        onTap: onTap,
      );
}
