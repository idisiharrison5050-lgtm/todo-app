...
class _TaskListViewState extends State<TaskListView> {
        ...
        slivers: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(_headerText(active, completed, filtered.length), style: Theme.of(context).textTheme.bodyMedium),
                      ],
                    ),
                  ),
          ...
          if (filtered.isEmpty)
            SliverPadding(
              ...
              sliver: SliverToBoxAdapter(child: _EmptyTasks(filter: widget.filter, searching: _query.isNotEmpty || _priorityFilter != null)),
            )
  ...
  bool _matchesFilter(Task task, DateTime now) {
    ...
    if (widget.filter == TaskFilter.upcoming) {
      ...
      return due.year == currentMinute.year && due.month == currentMinute.month && due.day == currentMinute.day
          ? !due.isBefore(currentMinute)
          : due.isAfter(today);
    }
    return false;
  }
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Text(keys[index], style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)),
              ),
              ...groups[keys[index]]!.map((task) => Padding(
                padding: const EdgeInsets.only(bottom: 10),
                child: _TaskCard(task: task, store: store, now: DateTime.now(), showOverdue: false),
              )),
            ],
          ),
        ),
...
class _EmptyTasks extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final text = searching ? ('No matching tasks', 'Try a different title, note, or priority.') : switch (filter) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(border: Border.all(color: Theme.of(context).colorScheme.outlineVariant), borderRadius: BorderRadius.circular(24)),
  ...
  @override Widget build(BuildContext context) => SafeArea(
    child: ListView(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text('Todo keeps your tasks and reminders on this device.', style: Theme.of(context).textTheme.bodyMedium),
        const SizedBox(height: 24),
        Card(
          child: Column(
            children: const [
              ListTile(leading: Icon(Icons.notifications_active_outlined), title: Text('Notifications'), subtitle: Text('Reminder notifications are enabled for scheduled tasks.')),
              Divider(height: 1),
