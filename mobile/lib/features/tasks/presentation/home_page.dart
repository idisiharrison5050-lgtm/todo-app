      ...
      child: CustomScrollView(
        slivers: [
            ...
            sliver: SliverToBoxAdapter(
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(_headerText(active, completed, shownCount), style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant)),
                ])),
                if (widget.filter == TaskFilter.today) Chip(label: Text('$shownCount left')),
          ...
          if (filtered.isEmpty)
            ...
            SliverPadding(padding: const EdgeInsets.fromLTRB(20, 8, 20, 120), sliver: SliverList.separated(itemCount: filtered.length, itemBuilder: (context, index) => _TaskCard(task: filtered[index], store: widget.store, now: now, showOverdue: widget.filter == TaskFilter.today), separatorBuilder: (context, index) => const SizedBox(height: 10))),
        ],
      ),
    );
  }
...
class _TaskCard extends StatelessWidget {
  ...
  Future<void> _handleAction(BuildContext context, String action) async { switch (action) { case 'edit': _openDetails(context); break; case 'complete': await store.toggleCompleted(task.id); break; case 'reminder': _openDetails(context); break; case 'delete': if (await _confirmDelete(context)) await store.deleteTask(task.id); break; } }
