...
class DashboardPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final overdue = tasks.where((task) => !task.isCompleted && task.dueAt != null && task.dueAt!.isBefore(DateTime.now())).length;
    final upcoming = tasks.where((task) => !task.isCompleted && task.dueAt != null && task.dueAt!.isAfter(DateTime.now())).toList()..sort((a, b) => a.dueAt!.compareTo(b.dueAt!));
    final rate = tasks.isEmpty ? 0.0 : completed / tasks.length;
    final scheme = Theme.of(context).colorScheme;

    return Scaffold(
      body: SafeArea(child: CustomScrollView(slivers: [
        SliverPadding(padding: const EdgeInsets.fromLTRB(20, 28, 20, 12), sliver: SliverToBoxAdapter(child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_greeting(), style: Theme.of(context).textTheme.bodyMedium), const SizedBox(height: 4), Text('Your productivity', style: Theme.of(context).textTheme.headlineMedium)])),
        ...
        SliverPadding(padding: const EdgeInsets.fromLTRB(20, 4, 20, 12), sliver: SliverToBoxAdapter(child: Card(color: scheme.primary, child: Padding(padding: const EdgeInsets.all(22), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 14), LinearProgressIndicator(value: rate, minHeight: 8, backgroundColor: scheme.onPrimary.withValues(alpha: .18), color: scheme.onPrimary),
          const SizedBox(height: 14), Text('$completed of ${tasks.length} tasks completed', style: TextStyle(color: scheme.onPrimary.withValues(alpha: .82), fontWeight: FontWeight.w600)),
        ])))),
        SliverPadding(padding: const EdgeInsets.fromLTRB(20, 8, 20, 12), sliver: SliverGrid(delegate: SliverChildListDelegate([
...
class _EmptyDashboard extends StatelessWidget {
  const _EmptyDashboard({required this.onAdd}); final VoidCallback onAdd;
