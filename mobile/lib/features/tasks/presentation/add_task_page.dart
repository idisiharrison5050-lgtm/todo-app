...
class _AddTaskPageState extends State<AddTaskPage> {
  ...
  Future<void> _pickDateTime() async {
    ...
    final same = date.year == now.year && date.month == now.month && date.day == now.day;
    final base = same ? now.add(const Duration(minutes: 2)) : (_dueAt ?? DateTime(date.year, date.month, date.day, 9));
    final time = await _scrollTime(base, same ? now.add(const Duration(minutes: 1)) : null);
    if (time == null || !mounted) return;
    final value = DateTime(date.year, date.month, date.day, time.hour, time.minute);
    if (same && !value.isAfter(now)) {
  ...
  Future<TimeOfDay?> _scrollTime(DateTime initial, DateTime? minimum) async {
    var h = initial.hour % 12;
    if (h == 0) h = 12;
    var m = initial.minute;
    var p = initial.hour < 12 ? 0 : 1;
    return showModalBottomSheet<TimeOfDay>(
      context: context,
      showDragHandle: true,
      builder: (ctx) {
        final hc = FixedExtentScrollController(initialItem: h - 1);
        final mc = FixedExtentScrollController(initialItem: m);
        final pc = FixedExtentScrollController(initialItem: p);
        return StatefulBuilder(builder: (ctx, setSheet) {
          final hour24 = p == 0 ? h % 12 : h % 12 + 12;
          final candidate = DateTime(initial.year, initial.month, initial.day, hour24, m);
          final valid = minimum == null || candidate.isAfter(DateTime(minimum.year, minimum.month, minimum.day, minimum.hour, minimum.minute));
          return SafeArea(child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(minimum == null ? 'Scroll to choose a time.' : 'Choose a future time.'),
              const SizedBox(height: 12),
              SizedBox(height: 190, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                _wheel(hc, 12, (i) => '${i + 1}', (i) => setSheet(() => h = i + 1)),
                const Text(':', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
                _wheel(mc, 60, (i) => i.toString().padLeft(2, '0'), (i) => setSheet(() => m = i)),
                const SizedBox(width: 12),
                _wheel(pc, 2, (i) => i == 0 ? 'AM' : 'PM', (i) => setSheet(() => p = i)),
              ])),
              const SizedBox(height: 8),
              SizedBox(width: double.infinity, child: FilledButton(onPressed: valid ? () => Navigator.pop(ctx, TimeOfDay(hour: hour24, minute: m)) : null, child: const Text('Set time'))),
            ]),
          ));
        });
      },
  ...
  Widget _wheel(FixedExtentScrollController c, int count, String Function(int) label, ValueChanged<int> onChanged) {
    return SizedBox(width: 70, child: ListWheelScrollView.useDelegate(
      controller: c,
      itemExtent: 48,
      ...
      childDelegate: ListWheelChildBuilderDelegate(
        childCount: count,
        builder: (_, i) => Center(child: Text(label(i), style: const TextStyle(fontSize: 23, fontWeight: FontWeight.w700))),
      ),
    ));
  }

  void _quick(Duration d) {
    final v = DateTime.now().add(d);
    setState(() {
      _dueAt = DateTime(v.year, v.month, v.day, v.hour, v.minute);
      if (_reminderType == TaskReminderType.none) _reminderType = TaskReminderType.once;
    });
  }

  void _tomorrow() {
    final v = DateTime.now().add(const Duration(days: 1));
    setState(() {
      _dueAt = DateTime(v.year, v.month, v.day, 9);
      if (_reminderType == TaskReminderType.none) _reminderType = TaskReminderType.once;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_reminderType == TaskReminderType.interval) {
      children.add(DropdownButtonFormField<Duration>(initialValue: _interval, items: const [DropdownMenuItem(value: Duration(minutes: 30), child: Text('Every 30 minutes')), DropdownMenuItem(value: Duration(hours: 1), child: Text('Every hour')), DropdownMenuItem(value: Duration(hours: 2), child: Text('Every 2 hours')), DropdownMenuItem(value: Duration(hours: 4), child: Text('Every 4 hours'))], onChanged: (v) => setState(() => _interval = v)));
    }
    children.addAll([
