...
class _AddTaskPageState extends State<AddTaskPage> {
  ...
  @override
  void dispose() {
    _title.dispose(); _notes.dispose(); _category.dispose(); _tag.dispose(); _customDays.dispose(); super.dispose();
  }
  ...
  String _scheduleLabel(BuildContext context) {
    ...
    return '${MaterialLocalizations.of(context).formatMediumDate(_dueAt!)} · ${TimeOfDay.fromDateTime(_dueAt!).format(context)}';
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          _Card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Icon(Icons.tune_rounded, color: scheme.primary), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Details', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900)), Text(_advanced ? 'Fine-tune how this task behaves.' : 'Priority, repeat, list and tags.', style: theme.textTheme.bodySmall)])), IconButton(onPressed: () => setState(() => _advanced = !_advanced), icon: Icon(_advanced ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded))]),
            if (_advanced) ...[
...
class _Chip extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => ActionChip(label: Text(label), onPressed: onTap);
}

class _Choice extends StatelessWidget {
  const _Choice({required this.title, required this.selected, required this.onTap});
