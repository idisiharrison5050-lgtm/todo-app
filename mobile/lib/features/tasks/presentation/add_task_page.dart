  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(20, 8, 20, 110), children: [
        ...
        _SurfaceSection(child: Column(children: [
          Wrap(spacing: 8, runSpacing: 8, children: [_QuickChip(label: '15 min', icon: Icons.bolt_rounded, onTap: () => _quick(const Duration(minutes: 15))), _QuickChip(label: '1 hour', icon: Icons.schedule_rounded, onTap: () => _quick(const Duration(hours: 1))), _QuickChip(label: 'Tomorrow', icon: Icons.wb_sunny_outlined, onTap: _tomorrow), _QuickChip(label: 'Pick date', icon: Icons.calendar_today_outlined, onTap: _pickDateTime)]),
          const SizedBox(height: 12),
          ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2), leading: CircleAvatar(backgroundColor: scheme.primary.withValues(alpha: .1), foregroundColor: scheme.primary, child: const Icon(Icons.event_available_outlined)), title: Text(_scheduleLabel(context), style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: Text(_dueAt == null ? 'Tap to schedule' : 'Tap to change', style: theme.textTheme.bodySmall), trailing: _dueAt == null ? const Icon(Icons.chevron_right_rounded) : IconButton(tooltip: 'Clear schedule', onPressed: () => setState(() { _dueAt = null; _reminderType = TaskReminderType.none; }), icon: const Icon(Icons.close_rounded)), onTap: _pickDateTime),
        ])),
        const SizedBox(height: 20), const _SectionHeading(icon: Icons.notifications_active_outlined, title: 'Reminder', subtitle: 'Never rely on memory alone.'), const SizedBox(height: 10),
        _SurfaceSection(child: Column(children: [
        ...
        _SurfaceSection(child: Column(children: [
          const _SectionHeading(icon: Icons.tune_rounded, title: 'Details', subtitle: 'Priority, repeat, list and tags.', compact: true), const SizedBox(height: 8),
        ...
        const SizedBox(height: 20),
        FilledButton.icon(onPressed: _saving ? null : _save, icon: _saving ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : Icon(widget.isEditing ? Icons.check_rounded : Icons.add_rounded), label: Padding(padding: const EdgeInsets.symmetric(vertical: 5), child: Text(_saving ? 'Saving…' : widget.isEditing ? 'Save changes' : 'Create task'))),
      ]),
    );
  }
}
...
class _WheelTimePickerState extends State<_WheelTimePicker> {
  late int _hour; late int _minute; late final FixedExtentScrollController _hourController; late final FixedExtentScrollController _minuteController;
