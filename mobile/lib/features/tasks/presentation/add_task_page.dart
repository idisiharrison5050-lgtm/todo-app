...
class _AddTaskPageState extends State<AddTaskPage> {
    ...
    try {
      ...
      if (widget.task == null) {
        await widget.store.addTask(
          title: args['title'], notes: args['notes'], dueAt: args['dueAt'],
          reminderType: args['reminderType'], reminderInterval: args['reminderInterval'],
          priority: args['priority'], repeat: args['repeat'],
          repeatIntervalDays: args['repeatIntervalDays'], isFavorite: args['isFavorite'],
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(error is ArgumentError ? error.message.toString() : 'Could not save the task. Try again.')),
      );
    }
  }
  ...
  String _scheduleLabel(BuildContext context) => _dueAt == null
      ? 'No date or time'
      : '${MaterialLocalizations.of(context).formatMediumDate(_dueAt!)} • ${TimeOfDay.fromDateTime(_dueAt!).format(context)}';
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const _SectionHeading(icon: Icons.schedule_rounded, title: 'When', subtitle: 'Set a moment or leave it open-ended.'),
                ...
                ListTile(
                  ...
                  trailing: _dueAt == null
                      ...
                      : IconButton(
                          ...
                          onPressed: () => setState(() { _dueAt = null; _reminderType = TaskReminderType.none; }),
          ...
          const SizedBox(height: 20),
          const _SectionHeading(icon: Icons.notifications_active_outlined, title: 'Reminder', subtitle: 'Never rely on memory alone.'),
          const SizedBox(height: 10),
          _SurfaceSection(
            child: Column(
              children: [
          ...
          _SurfaceSection(
            child: Column(
              children: [
                const _SectionHeading(icon: Icons.tune_rounded, title: 'Details', subtitle: 'Priority, repeat, list and tags.', compact: true),
                ...
                if (_showAdvanced) ...[
                  ...
                  DropdownButtonFormField<TaskRepeat>(
                    ...
                    onChanged: (value) => setState(() => _repeat = value ?? TaskRepeat.none),
                  ),
                  if (_repeat == TaskRepeat.custom) ...[
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(labelText: 'Repeat every N days', prefixIcon: Icon(Icons.timelapse_rounded)),
                    ),
                  ],
                  const SizedBox(height: 12),
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'List / category', hintText: 'Work, Personal, Study', prefixIcon: Icon(Icons.folder_outlined)),
                  ),
                  const SizedBox(height: 12),
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Add tag', hintText: 'work, study, personal', prefixIcon: Icon(Icons.tag), suffixIcon: Icon(Icons.add_rounded)),
                  ),
                  if (_tags.isNotEmpty) ...[
                    const SizedBox(height: 10),
                    Align(
                      ...
                      child: Wrap(
                        ...
                        children: [
                          for (final tag in _tags)
                            InputChip(label: Text(tag), onDeleted: () => setState(() => _tags.remove(tag))),
                        ],
                      ),
                    ),
                  ],
                  SwitchListTile(
                    ...
                    subtitle: const Text('Keep this task close at hand'),
                    value: _favorite,
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(child: Text('Ready: ${_scheduleLabel(context)}${_reminderType == TaskReminderType.none ? '' : ' • ${_reminderLabel()}'}', style: const TextStyle(fontWeight: FontWeight.w700))),
                ],
              ),
            ),
          const SizedBox(height: 20),
...
class _SectionHeading extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(children: [
      if (!compact) ...[
        CircleAvatar(radius: 19, backgroundColor: scheme.primary.withValues(alpha: .1), foregroundColor: scheme.primary, child: Icon(icon, size: 20)),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 18),
          Row(children: [
            Expanded(child: Text('Set time', style: Theme.of(context).textTheme.titleLarge)),
                ...
                IgnorePointer(
                  child: Center(
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: scheme.primary.withValues(alpha: .06), borderRadius: BorderRadius.circular(16), border: Border.symmetric(horizontal: BorderSide(color: scheme.primary.withValues(alpha: .12)))),
                    ),
                  ),
                ),
                Row(children: [
                  Expanded(child: _wheel(controller: _hourController, count: 24, selected: _hour, onChanged: (value) => setState(() => _hour = value))),
                  SizedBox(width: 24, child: Center(child: Text(':', style: Theme.of(context).textTheme.headlineSmall))),
    ...
    return ListWheelScrollView.useDelegate(
      ...
      childDelegate: ListWheelChildBuilderDelegate(
        ...
        builder: (context, index) {
          ...
          final opacity = distance == 0 ? 1.0 : distance == 1 ? .38 : .12;
          return Center(
            child: Opacity(
              opacity: opacity,
              child: Text(_twoDigits(index), style: Theme.of(context).textTheme.displaySmall?.copyWith(fontSize: 48, fontWeight: FontWeight.w300)),
            ),
          );
        },
      ),
