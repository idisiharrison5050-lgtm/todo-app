    ...
    try {
      ...
      if (metadata != null) {
        ...
        for (final operation in operations) {
          if (operation.type == PendingOperationType.delete) {
            int? version;
            if (_local is LocalTaskDatabase) version = await (_local as LocalTaskDatabase).getPendingDeleteVersion(operation.id);
      ...
      for (final task in local) {
        ...
        if (localVersion != null && remoteVersion != null) {
          if (remoteVersion > localVersion) await _local.saveTask(remoteTask);
          ...
          if (localTime != null && remoteTime != null && remoteTime.isAfter(localTime)) await _local.saveTask(remoteTask);
          else {
            final serverTask = await _cloud.push(task);
            if (serverTask == null) await _local.deleteTask(task.id); else await _local.saveTask(serverTask);
          }
