...
class SyncingTaskRepository implements TaskRepository {
  ...
  SyncMetadataStore? get _metadata {
    if (_local is SyncMetadataStore) {
      return _local;
    }
    return null;
  }
  ...
  Future<void> syncNow() async {
    ...
    try {
      ...
      final remoteById = <String, Task>{
        for (final task in remote) task.id: task,
      };
      final pendingIds = metadata == null
          ? <String>{}
          : (await metadata.getPendingOperations())
  ...
  Future<void> saveTask(Task task) async {
    ...
    final metadata = _metadata;
    if (metadata != null) {
      await metadata.markPendingUpsert(
        changedTask.id,
        changedTask.updatedAt!,
      );
    }
    state.value = state.value.copyWith(
      status: SyncStatus.syncing,
  ...
  Future<void> deleteTask(String id) async {
    ...
    final metadata = _metadata;
    if (metadata != null) {
      await metadata.addPendingDelete(id);
    }
    await _local.deleteTask(id);
    try {
      ...
      if (error.response?.statusCode != null &&
        ...
        state.value = state.value.copyWith(
          status: SyncStatus.error,
          message: 'Deleted locally — will sync later',
        );
      } else {
        state.value = state.value.copyWith(
          status: SyncStatus.offline,
