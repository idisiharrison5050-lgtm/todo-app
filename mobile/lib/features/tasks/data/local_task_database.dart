...
class LocalTaskDatabase implements TaskRepository, SyncMetadataStore {
  ...
  @override Future<void> addPendingDelete(String id) async { final key = await _accountKey(); await (await _db).insert(_deletedTable, {'id': id, 'account_key': key, 'deleted_at': DateTime.now().toIso8601String()}, conflictAlgorithm: ConflictAlgorithm.replace); }
  @override Future<void> clearPendingDelete(String id) async { final key = await _accountKey(); await (await _db).delete(_deletedTable, where: 'id = ? AND account_key = ?', whereArgs: [id, key]); }
