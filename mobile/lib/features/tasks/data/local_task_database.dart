...
class LocalTaskDatabase implements TaskRepository, SyncMetadataStore {
  ...
  Future<String> _accountKey() async {
    ...
    final key = accountId != null && accountId.isNotEmpty
        ? 'account:$accountId'
        : token == null || token.isEmpty
            ? 'anonymous'
            : 'token:${sha256.convert(utf8.encode(token)).toString()}';
  ...
  Future<void> addPendingDelete(String id) async {
    final key = await _accountKey();
    final db = await _db;
    await db.insert(_deletedTable, {'id': id, 'account_key': key, 'deleted_at': DateTime.now().toUtc().toIso8601String()}, conflictAlgorithm: ConflictAlgorithm.replace);
    await db.insert(_operationsTable, {'id': id, 'account_key': key, 'operation': 'delete', 'updated_at': DateTime.now().toUtc().toIso8601String()}, conflictAlgorithm: ConflictAlgorithm.replace);
  }

  @override
  Future<void> clearPendingDelete(String id) async {
    await clearPendingOperation(id);
  }
