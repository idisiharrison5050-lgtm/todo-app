...
class CloudTaskSync {
  ...
  Future<bool> get isAuthenticated async =>
      (await _storage.read())?.isNotEmpty == true;
      ...
      if (raw is List) {
        tasks.addAll(
          raw.whereType<Map>().map((item) {
            ...
            if (payload is Map) {
              final decoded =
                  Task.fromJson(Map<String, dynamic>.from(payload));
              return decoded.copyWith(
                syncVersion:
                    data['sync_version'] as int? ?? decoded.syncVersion,
              );
            }
            return Task(
              ...
              syncVersion: data['sync_version'] as int?,
            );
  ...
  Task _taskFromResponse(dynamic response, Task fallback) {
    if (response is Map) {
      ...
      if (data is Map) {
        ...
        return Task(
          id: data['client_id'] as String? ?? fallback.id,
          title: data['title'] as String? ?? fallback.title,
          isCompleted:
              data['completed'] as bool? ?? fallback.isCompleted,
          updatedAt: DateTime.tryParse(
                data['updated_at'] as String? ??
                    data['client_updated_at'] as String? ??
                    '',
              ) ??
              fallback.updatedAt,
          syncVersion:
              data['sync_version'] as int? ?? fallback.syncVersion,
        );
      }
    }
    return fallback;
  ...
  Options _options(String token, {String? operationId}) => Options(
        headers: {
