...
class TaskApiTest extends TestCase
{
    ...
    public function test_user_can_create_and_list_their_tasks(): void
    {
        ...
        $response->assertCreated()
            ->assertJsonPath('task.client_id', 'task-001')
            ->assertJsonPath('task.title', 'Buy groceries')
            ->assertJsonPath('task.payload.id', 'task-001');

        $this->getJson('/api/v1/tasks')
            ->assertOk()
            ->assertJsonPath('data.0.client_id', 'task-001');
    }
    ...
    public function test_user_cannot_read_another_users_tasks(): void
    {
        ...
        Task::create([
            'user_id' => $owner->id,
            'client_id' => 'private-task',
            'title' => 'Private task',
            'completed' => false,
            'payload' => ['id' => 'private-task', 'title' => 'Private task'],
        ]);
    ...
    {
        $user = User::factory()->create();
        $serverTime = Carbon::parse('2026-09-04T08:00:00Z');
        $task = Task::create([
            'user_id' => $user->id,
            'client_id' => 'conflict-task',
            'title' => 'Server version',
            'completed' => true,
            'payload' => ['id' => 'conflict-task', 'title' => 'Server version', 'isCompleted' => true],
        ...
        $this->postJson('/api/v1/tasks', [
            'client_id' => 'conflict-task',
            'title' => 'Older local version',
            'completed' => false,
            'payload' => ['id' => 'conflict-task', 'title' => 'Older local version', 'isCompleted' => false],
            'client_updated_at' => $serverTime->copy()->subMinute()->toIso8601String(),
        ])->assertStatus(409)
            ->assertJsonPath('task.client_id', 'conflict-task')
            ->assertJsonPath('task.title', 'Server version')
    ...
    {
        $user = User::factory()->create();
        $createdAt = Carbon::parse('2026-09-04T07:00:00Z');
        Task::create([
            'user_id' => $user->id,
            'client_id' => 'deleted-task',
            'title' => 'Delete me',
            'completed' => false,
            'payload' => ['id' => 'deleted-task', 'title' => 'Delete me'],
        ...
        $this->getJson('/api/v1/tasks/deleted')
            ->assertOk()
            ->assertJsonPath('data.0.client_id', 'deleted-task');

        $this->postJson('/api/v1/tasks', [
            'client_id' => 'deleted-task',
            'title' => 'Old offline copy',
            'completed' => false,
            'payload' => ['id' => 'deleted-task', 'title' => 'Old offline copy', 'updatedAt' => $createdAt->toIso8601String()],
